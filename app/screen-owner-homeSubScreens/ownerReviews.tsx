...
export default function OwnerReviewsScreen() {
    ...
    const {
        ...
        fetchOwnerReviews,
        loadMoreOwnerReviews,
        refreshOwnerReviews
    } = useReviewStore();

    useEffect(() => {
        fetchOwnerReviews({ forceRefresh: true });
    }, []);

    const handleTabChange = useCallback((status: ReviewStatus) => {
        setSelectedOwnerStatus(status);
        fetchOwnerReviews({ page: 1, status, forceRefresh: true });
    }, [setSelectedOwnerStatus, fetchOwnerReviews]);
    ...
    const renderReviewCard = ({ item }: { item: Review }) => {
        ...
        const advertiserName = item.advertiser?.business_name || item.advertiser?.name || 'Unknown Advertiser';
        ...
        return (
            <View style={[styles.card, { backgroundColor: colorScheme === 'dark' ? '#1A1A1A' : '#FFFFFF' }]}>
                ...
                <Text style={[styles.advertiserName, { color: theme.textSecondary }]}>
                    By {advertiserName} {item.order?.order_number ? `(Order: ${item.order.order_number})` : ''}
                </Text>
        ...
        <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptySubtitle, { color: theme.textSecondary }]}>
                {selectedOwnerStatus === 'all'
                    ? "Your screens haven't received any reviews yet."
                    : `There are no ${selectedOwnerStatus} reviews for your screens.`}
            </Text>
        </View>
    );
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background, paddingTop: insets.top }]}>
            ...
            <View style={styles.header}>
                ...
                <Text style={[styles.headerTitle, { color: theme.text }]}>Screen Reviews</Text>
                <View style={{ width: 40 }} />
            </View>
            ...
            <View style={styles.tabsContainer}>
                {TABS.map((tab) => {
                    const isActive = selectedOwnerStatus === tab.id;
                    return (
                        <TouchableOpacity
                            key={tab.id}
                            style={[
            ...
            {loading && ownerReviews.length === 0 ? (
                ...
                <FlatList
                    ...
                    refreshControl={
                        <RefreshControl refreshing={refreshing} onRefresh={refreshOwnerReviews} tintColor={theme.tint} />
                    }
                    onEndReached={loadMoreOwnerReviews}
